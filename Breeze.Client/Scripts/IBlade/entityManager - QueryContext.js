...
//            });
        ...
        proto.saveChanges = function (entities, saveOptions, callback, errorCallback) {
            ...
            return deferred.promise.then(function (rawSaveResult) {
                ...
                var savedEntities = saveResult.entities.map(function (rawEntity) {
                    return queryContext.mergeEntity(rawEntity);
                });
                ...
                dataService.adapterInstance.executeQuery(em, odataQuery, function (data) {
                    var result = core.wrapExecution(function () {
                        ...
                        var entities = nodes.map(function (node) {
                            ...
                            var entity = queryContext.mergeEntity(node, true);
    ...
    var QueryContext = (function () {
        ...
        proto.mergeEntity = function (node, isTopLevel) {

            var em = this.entityManager;
            var mergeStrategy = this.mergeStrategy;
            var isSaving = this.query == null;
            // will have already been set if called nested.
            var meta = node._$meta;
            if (!meta) {
                meta = this.jsonResultsAdapter.visitObjectNode(node, this, isTopLevel);
            }
            ...
            if (meta.nodeRefId) {
                return this.resolveRefEntity(meta.nodeRefId);
            }
            ...
            if (entityType == null) {
                ...
                entityType = isTopLevel && this.query && this.query._getToEntityType && this.query._getToEntityType(em.metadataStore);
                if (!entityType) {
            ...
            if (targetEntity) {
                ...
                if (mergeStrategy === MergeStrategy.OverwriteChanges
                    ...
                    this.updateCurrentRef(targetEntity, node);
        ...
        proto.processAnonType = function (node) {
            var em = this.entityManager;
            var jsonResultsAdapter = this.jsonResultsAdapter;
            ...
            core.objectForEach(node, function (key, value) {
                ...
                var anonMeta = jsonResultsAdapter.visitAnonPropNode(value, that, key);

                if (anonMeta.ignore) return;
                if (anonMeta.nodeId) {
                    // value === anonMeta.nodeId
                    that.refMap[value] = result;
                    return;
                }
                ...
                if (value == null) {
                    ...
                    result[newKey] = value.map(function (v, ix, arr) {
                        ...
                        meta = jsonResultsAdapter.visitObjectNode(v, that);
                        if (meta.entityType) {
                            v._$meta = meta;
                            return that.mergeEntity(v);
                        } else if (meta.nodeRefId) {
                            refValue = that.resolveRefEntity(meta.nodeRefId);
                    ...
                    meta = jsonResultsAdapter.visitObjectNode(value, that);

                    if (meta.entityType) {
                        value._$meta = meta;
                        result[newKey] = that.mergeEntity(value);
                    } else if (meta.nodeRefId) {
                        refValue = that.resolveRefEntity(meta.nodeRefId);
                        if (typeof refValue == "function") {
                            that.deferredFns.push(function () {
        ...
        proto.mergeRelatedEntityCore = function (rawEntity, navigationProperty) {
            ...
            var navMeta = this.jsonResultsAdapter.visitNavPropNode(relatedRawEntity, this, navigationProperty);
            if (navMeta.ignore) return;

            var relatedEntity = this.mergeEntity(relatedRawEntity);
        ...
        proto.mergeRelatedEntitiesCore = function (rawEntity, navigationProperty) {
            ...
            var navMeta = this.jsonResultsAdapter.visitNavPropNode(relatedRawEntities, this, navigationProperty);
            ...
            var relatedEntities = relatedRawEntities.map(function (relatedRawEntity) {
                return that.mergeEntity(relatedRawEntity);
            });
        ...
        proto.updateEntity = function (targetEntity, rawEntity) {
            this.updateCurrentRef(targetEntity, rawEntity);
        ...
        proto.updateCurrentRef = function (targetEntity, rawEntity) {
