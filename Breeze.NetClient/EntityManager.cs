...
namespace Breeze.NetClient {
  public class EntityManager {
    ...
    private EntityAspect AttachEntityAspect(EntityAspect entityAspect, EntityState entityState) {
      ...
      LinkRelatedEntities(entityAspect);
      return entityAspect;
    }

    internal void LinkRelatedEntities(EntityAspect aspect) {
      //// we do not want entityState to change as a result of linkage.
      using (NewIsLoadingBlock()) {
        LinkUnattachedChildren(aspect);
        LinkNavProps(aspect);
        LinkFkProps(aspect);
      }
    }

    private void LinkUnattachedChildren(EntityAspect aspect) {
      var entity = aspect.Entity;
      var entityKey = aspect.EntityKey;
    ...
    private void LinkFkProps(EntityAspect aspect) {
    ...
    private void LinkNavProps(EntityAspect aspect) {
      ...
      aspect.EntityType.NavigationProperties.ForEach(np => {
        ...
        var parentKey = aspect.GetParentKey(np);
        if (parentKey != null) {
          ...
          if (parent != null) {
            ...
            // else add parent to unresolvedParentMap;
            UnattachedChildrenMap.AddChild(parentKey, np, aspect.Entity);
          }
        }
      });
    }
