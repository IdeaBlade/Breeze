...
namespace Breeze.WebApi {
  ...
  public class EFContextProvider<T> : ContextProvider where T : class, new() {
    ...
    private ObjectStateEntry HandleAdded(EFEntityInfo entityInfo) {
      AttachIndependentAssociationEntities(entityInfo);
    ...
    private ObjectStateEntry HandleModified(EFEntityInfo entityInfo) {
      ...
      DeleteOldIndependentAssociations(entry, entityInfo);
    ...
    private void AttachIndependentAssociationEntities(EntityInfo entityInfo)
    {
        Type entityType = entityInfo.Entity.GetType();
        foreach (NavigationProperty navigationProperty in entityInfo.ChangedIndependentAssociations)
        {
            string setName = GetEntitySetName(ObjectContext, navigationProperty.EntityType);

            PropertyInfo property = entityType.GetProperty(navigationProperty.PropertyName, BindingFlags.Instance | BindingFlags.Public);
            object propEntity = property.GetValue(entityInfo.Entity, null);

            ObjectContext.AttachTo(setName, propEntity);
        }
    }

    private void DeleteOldIndependentAssociations(ObjectStateEntry entry, EntityInfo entityInfo)
    {
        Type entityType = entityInfo.Entity.GetType();
        foreach (NavigationProperty navigationProperty in entityInfo.ChangedIndependentAssociations.Where(np => np.OldEntity != null))
        {
            string setName = GetEntitySetName(ObjectContext, navigationProperty.EntityType);

            ObjectContext.AttachTo(setName, navigationProperty.OldEntity);
            ObjectContext.ObjectStateManager.ChangeRelationshipState(entityInfo.Entity, navigationProperty.OldEntity, navigationProperty.PropertyName, System.Data.EntityState.Deleted);
        }
    }
