...
namespace Breeze.WebApi {
  ...
  public class EFContextProvider<T> : ContextProvider, IEFContextProvider where T : class, new() {
    ...
    public override object[] GetKeyValues(EntityInfo entityInfo) {
      // need to add the object to the ObjectContext in order to create an objectStateEntry in order to get the keyValues
      var objContext = this.ObjectContext;
      var entity = entityInfo.Entity;
      var esName = this.GetEntitySetName(entity.GetType());
      ...
      if (key.EntityKeyValues != null) {
        ...
        var ix = currentValues.GetOrdinal(entityInfo.AutoGeneratedKey.PropertyName);
        keyValues = new Object[] { currentValues[ix] };
      }
      return keyValues;
    }
